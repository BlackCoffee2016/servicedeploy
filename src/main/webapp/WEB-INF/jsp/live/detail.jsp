			...
			<div class="col s12">
				<div class="card">
					<div class="card-content">
						...
						<p>SVN地址：${detail.svn}</p>
						<p>启动参数：${detail.parameter}</p>
						<p>是否需要强制kill：${detail.forceKill}</p>
					...
					<div class="card-action">
						<p>
							...
							<%-- <a class="btn waves-light waves-effect white-text" href="${pageContext.request.contextPath}/live/delete?uuid=${detail.uuid}">删除</a> --%>
						</p>
					</div>
				</div>
			</div>
		</div>
		<h2 style="font-weight: 300; font-size: 1.64rem; line-height: 110%; margin: 0.82rem 0 0.656rem 0;">服务器列表：</h2>
		<div class="row">
			<c:forEach var="item" items="${detail.serverList}">
				<div class="col s12 m6 l4">
					<div class="card">
						<div class="card-content">
							<span class="card-title grey-text text-ip">${item}</span>
							<p>
								运行状态：
						...
						<div class="card-action">
							<a href="#" class="btn-showlog" data-ip="${item}">查看日志</a>
						</div>
					</div>
				</div>
			</c:forEach>
